        ...
        try:
            ...
            if app.config['SQLALCHEMY_DATABASE_URI'].startswith('postgresql'):
                ...
                with db.engine.connect().execution_options(isolation_level="AUTOCOMMIT") as conn:
                    ...
                    if transactions:
                        app.logger.warning(
                            f"Found {len(transactions)} potentially problematic "
                            "transactions. Resetting them...")
                        for tx in transactions:
                            ...
                            if tx.state != 'active':
                                try:
                                    ...
                                    app.logger.info(
                                        f"Terminated connection {tx.pid} "
                                        f"in state {tx.state}")
                                except Exception as e:
                                    app.logger.warning(
                                        f"Could not terminate connection {tx.pid}: "
                                        f"{str(e)}")
