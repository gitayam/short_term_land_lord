...
def view_calendar(id):
    ...
    # Add additional safety checks for template rendering
    try:
        # Ensure calendars is a list
        if not isinstance(calendars, list):
            calendars = list(calendars) if calendars else []
        
        # Ensure events is a list
        if not isinstance(events, list):
            events = []
        
        # Validate that property object has required attributes
        if not hasattr(property, 'name'):
            current_app.logger.error(f"Property {id} missing name attribute")
            property.name = f"Property #{id}"
        
        current_app.logger.info(f"Rendering calendar view for property {property.id} with {len(calendars)} calendars and {len(events)} events")
        
        return render_template('property/calendar_view.html', 
                             property=property, 
                             calendars=calendars, 
                             events=events)
    except Exception as render_error:
        current_app.logger.error(f"Error rendering calendar template: {str(render_error)}")
        flash(f'Error displaying calendar: {str(render_error)}', 'danger')
